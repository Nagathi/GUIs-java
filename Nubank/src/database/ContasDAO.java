...
public class ContasDAO {
	...
	public void cadastrar_conta(Conta objConta) {
		...
		String sql = "INSERT INTO contas (nome, nascimento, cpf, usuario, senha, tipo, emprestimo, dinheiro, mensal, contaMensal, fatura) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try {
			...
			pstm.setBoolean(7, objConta.getEmprestimo());
			pstm.setFloat(8, objConta.getDinheiro());
			pstm.setFloat(9, objConta.getMensal());
			pstm.setInt(10, objConta.getContaMensal());
			pstm.setFloat(11, objConta.getFatura());
	...
	public ArrayList<Conta> listar_conta() {
		...
		try {
			...
			while(resSet.next()) {
				...
				objConta.setFatura(resSet.getFloat("fatura"));
	...
	public void alterar_conta(Conta objConta) {
		...
		String sql = "UPDATE contas SET dinheiro = ?, fatura = ?, mensal = ?, emprestimo = ?  WHERE usuario = ?";
		...
		try {
			...
			pstm.setFloat(2, objConta.getFatura());
			pstm.setFloat(3, objConta.getMensal());
			pstm.setBoolean(4, objConta.getEmprestimo());
			pstm.setString(5, objConta.getUsuario());
